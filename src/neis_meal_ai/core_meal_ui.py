...
from .mokpo_analytics import high_school_side_dish_values
...
def create_mokpo_app(
    ...
    *,
    ...
    side_dish_values = high_school_side_dish_values(dataset.meals)

    with gr.Blocks(title="오늘 점심 취향 예측") as demo:
        with gr.Tab("오늘 점심 취향 예측"):
            ...
            gr.Markdown("---\n## 2. 오늘 점심 예상하기")
            meal_choice = gr.Radio(
                choices, value=default_menu_id, label="점심 날짜"
            )
            meal_summary = gr.Markdown(
                _menu_summary(validation_menus, default_menu_id)
            )
            predict_button = gr.Button("오늘 점심 예상하기", variant="primary")
            prediction_message = gr.Markdown(
                "30개 평점을 저장한 뒤 예상하기를 누르세요."
            )
            dish_table = gr.Dataframe(label="메뉴별 예상과 근거", interactive=False)

            gr.Markdown("---\n## 3. 식사 후 실제 만족도 비교")
            actual_rating = gr.Slider(
                1, 5, value=3, step=1, label="실제 만족도(1~5)"
            )
        ...
        with gr.Tab("고등학교 주요 반찬"):
            gr.Markdown("## 전체 고등학교 주요 반찬(TF-IDF)")
            gr.Markdown(
                "목포 지역 고등학교 급식 전체를 합쳐, 밥·국·김치·후식류를 제외한 주요 반찬을 보여 줍니다.  \n"
                "높은 점수는 자주 나오면서 모든 학교에 흔하지 않은 반찬이라는 뜻이며 맛·영양 순위는 아닙니다."
            )
            gr.Dataframe(
                value=side_dish_values,
                label="전체 고등학교 주요 반찬(TF-IDF)",
                interactive=False,
            )
            gr.Markdown(
                "`전체 TF = 반찬 등장 횟수 ÷ 전체 반찬 수`  \n"
                "`IDF = ln((1 + 고등학교 수) ÷ (1 + 해당 반찬이 나온 학교 수)) + 1`  \n"
                "`TF-IDF = 전체 TF × IDF`"
            )
    return demo
