...
| 3 | 문자 n-gram, TF·DF·IDF, 코사인 유사도, K-Means | 손으로 확인한 계산과 취향·메뉴 비교 결과 |
...
메뉴 문장은 2~4글자 n-gram으로 나뉩니다. TF는 한 메뉴 안에서 각 조각이 차지하는 비율을, DF는 그 조각이 들어 있는 메뉴 수를 셉니다. IDF는 여러 메뉴에서 드문 조각에 더 큰 값을 주고, TF와 IDF를 곱한 값들을 모아 TF-IDF 벡터를 만듭니다. 코사인 유사도는 가상 취향 문장과 각 메뉴의 벡터 방향을 비교합니다. 여기에 좋아하는 항목, 피하고 싶은 항목, 메뉴 종류, 매운맛 차이를 더해 최종 순서를 정합니다.
